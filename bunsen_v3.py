...
def bunsen_take_beaker(new_rand_beak_cart_arr):
    ...
    crx10_bunsen.write_cartesian_position(
        new_rand_beak_cart_arr
        + mid_beak_cart_arr
        + mid_bun_offset_arr
        + bun_safe_offset_arr
    )
    crx10_bunsen.write_cartesian_position(
        new_rand_beak_cart_arr + mid_beak_cart_arr + mid_bun_offset_arr
    )
...
def on_message(client, userdata, message):
    ...
    if not has_dice and "location" in message_json:
        ...
        crx10_bunsen.write_cartesian_position(
            np.array(crx10_bunsen.read_current_cartesian_pose()) + bun_safe_offset_arr
        )
        send_gripper_status(
            client, topic_bunsen, "bunsen", gripper_closed
        )  # Notify Beaker
